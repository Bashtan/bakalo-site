...
export async function onRequestPut({ request, env, params }) {
  ...
  const existing = await env.DB.prepare('SELECT * FROM engagements WHERE id = ?').bind(params.id).first();
  ...
  if (body.organization !== undefined) updated.organization = body.organization;
...
export async function onRequestDelete({ request, env, params }) {
  ...
  await env.DB.prepare('DELETE FROM engagements WHERE id = ?').bind(params.id).run();
