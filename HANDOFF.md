...
| 2026-07-08 | Floating glassmorphism nav redesign — replaced the flat, full-width, opaque nav bar with a floating pill header (`position:fixed`, rounded, translucent `backdrop-filter: blur()` background, subtle shadow/border); consolidated the 9 flat links into Home, About, two hover/click-toggle dropdowns ("Research & Impact": Research Overview/Research Profiles/Implementation & Recognition/Research Impact; "EB-2 NIW Vision": Proposed Endeavor/Professional Engagement), and a distinct gold-bordered Contact CTA button; mobile collapses into a full-screen dark overlay with accordion-style dropdown groups; kept hand-written CSS (no Tailwind) and the Research Impact dropdown item, per two AskUserQuestion confirmations against the client's generic redesign brief; branched from `feature/research-impact` (not `main`) since `#impact` only exists there; fixed a bug where the mobile media query's `.nav-cta { display:none }` was also hiding the CTA duplicated inside the mobile overlay (now scoped to `nav > .nav-cta`); note: `backdrop-filter` triggers a blank-frame artifact in the preview tool's headless screenshot capture (not a real rendering bug — confirmed by toggling it off, which produced a normal screenshot; unaffected in real browsers); 77 tests still green; pushed to `fix/nav-glassmorphism` |
