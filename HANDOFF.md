...
| 2026-07-08 | Research Profiles card refactor (`fix/profile-cards-refactor`, based on `main`) — the 5 platform cards (SSRN, Google Scholar, ORCID, ResearchGate, LinkedIn) are now fully-clickable `<a>` elements (no more "View Profile" buttons), with a `translateY(-4px)` + shadow hover effect for affordance; outlined `.stat-chip` metric pills replaced with minimalist `.stat-row` ledger rows (label left, value right, subtle bottom-border divider, none on last row) for a cleaner report-style presentation; `renderProfileStats()` JS updated to emit the new row markup; kept hand-written CSS (no Tailwind) per the project's locked single-file architecture; 61/61 tests green; pushed to staging |
| 2026-07-08 | Cherry-picked the floating glassmorphism nav redesign (commit `af30d34` from `fix/nav-glassmorphism`) onto `fix/profile-cards-refactor` so the staging preview shows both features together; resolved one conflict (removed a leftover flat `<ul class="nav-links">` remnant that had already been superseded by the new nav markup); verified dropdowns, mobile hamburger/accordion overlay, and CTA responsiveness all work correctly alongside the refactored profile cards; note — `preview_screenshot` renders a blank frame whenever the page is scrolled past the fixed `backdrop-filter` nav (known headless-Chromium compositing artifact, confirmed non-issue via DOM/computed-style/bounding-box inspection and via real screenshots at scrollY=0) |
| 2026-07-08 | Edit Stats admin button visibility bug fix (ported from `fix/impact-dashboard-restyle`) — client pointed at this branch's staging URL asking to "implement all changes"; since the branch is unrelated to the impact-dashboard work, scope was clarified with the client, who confirmed only this bug fix should be ported; root cause: `.btn-accent { display:none }` was never overridden for `#btnEditProfileStats` because `activateAdmin()`/`deactivateAdmin()` toggled its inline `style.display` directly instead of using the `.visible` class pattern every other admin button uses (inline styles beat class rules); fixed by removing the dead inline `style="display:none"` from the HTML and adding `btnEditProfileStats` to the same `.visible`-class toggle array in both functions; verified via direct `activateAdmin()`/`deactivateAdmin()` calls (button now correctly shows/hides, real bounding box, no console errors); 61/61 tests green; pushed to `fix/profile-cards-refactor`, CI deploy succeeded, confirmed live on staging |
